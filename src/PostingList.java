...
public class PostingList {
	...
	public static void postingList(List<Node>nodes,String []term,String article){
		if(nodes!=null){
			for(int i =0;i<term.length;i++){
				if(isRepeat(nodes,term[i],article)){//将重复的放到之前的node中
					modifyNode(nodes, term[i], article);
				}else{//同一篇文章中有重复单词不用加，不同文章出现重复单词才加
	...
	static boolean isRepeat(List<Node>nodes,String word,String article){			
	...
	static boolean isInNode(Node node,String article){
	...
	static void addNewNode(List<Node>nodes,String word,String article){
		Node node = new Node(word);
		node.appendNum(id);
		node.appendNum(1);
		node.appendWord(article);
		nodes.add(node);
		id++;
	}
	
	static void modifyNode(List<Node>nodes,String word,String article){
		for(int i =0;i<nodes.size();i++){
			if(nodes.get(i).word.equals(word)){
				int freq =nodes.get(i).getFreq();
				freq++;
				nodes.get(i).modifyFreq(freq);
				nodes.get(i).appendWord(article);
			}
		}
	}
}
